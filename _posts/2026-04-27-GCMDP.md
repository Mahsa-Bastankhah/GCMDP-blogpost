...
<!-- Other than the expected discounted return, RL probably typically measure the expected discounted return starting from a stat-action pair $(s, a)$ and a state $s$ as the Q-function and the value function respective. -->



<!-- we measure the expected
discounted return starting from a state-action
pair (s, a) and a state s as the (unnormalized)
Q-function and the value function, respectively -->
...
<!-- The expected discounted return under a policy $$\pi(a \mid s)$$ is:
...
<span style="color: #1565C0; font-weight: 600;">📝 Note on Extensions:</span>
...
**Figure 1.** Success rate of DQN, PPO, SGCRL, and oracle in the augmented GC-MDP. Success rate is averaged over 5 seeds with standard error shown.
